...
function setupTestHelpers() {
    ...
    this.newMediumEditorMention = function (selector, options) {
        var mentionPlugin = new MediumEditorMention(selector, options);
        this.mentionPlugins.push(editor);
        return mentionPlugin;
    };

    this.cleanupTest = function () {
        this.mentionPlugins.forEach(function (editor) {
            mentionPlugins.destroy();
        });
